...
!!! info "Identifiers and quasi-Identifiers in the metadata model"
  Identifiers are unique data elements that can directly link a record to an individual or entity, such as an accession number or a participant ID. Quasi-identifiers, by contrast, are attributes that may not identify an individual on their own but could leat to re-identification when combined with other data points. In the GHGA metadata model, these are sex, age, diagnosis, phenotypic feature, ancestry, and geographic location. While the model employs privacy by design, minimizing the collection and exposure of identifiers and quasi-identifiers, submitters should be aware that rare combinations of quasi-identifiers could lead to data subject re-identification.
