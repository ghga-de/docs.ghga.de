...
:   An agreement signed between a Research Data Controller and a Data Requester before a Data Access Request is approved. GHGA is not involved in the contractual relationship here, but is informed by the Research Data Controller once an appropriate agreement is in place. Please note: Depending on the institutions, this is sometimes also referred to as a Data Access Agreement (DAA) or Data Use Agreement (DUA). Further information [here](user_stories/dua-guideline.md).

general:DKFZ Internal Terms of Use of the GHGA Data Infrastructure (ITOU)
:    For submissions to GHGA where DKFZ is the controller, the DKFZ Internal Terms of Use (ITOU) replace the <general:Data Processing Contract (DPC)> as DKFZ cannot sign a contract with itself. Similar to the DPC, the DKFZ Internal Terms of Use explain the relationship and roles between GHGA and the Data Submitter. Instructions to DKFZ users are given via [the ODCF-Website](https://odcf.dkfz.de/supporting-information/data-transfers/ghga).
