...
GHGA Connector requires a [Crypt4GH](https://crypt4gh.readthedocs.io/latest/) key pair to upload or download data. Please create a pair of Crypt4GH keys if you don't already have one. The public key is also needed for the creation of a download token or an upload box access token through the Data Portal.
...
Installation of C4GH and generation of keys:
...
To submit Research Data Files as part of a [GHGA Submission](../user_stories/submission/submitter_guide.md), use the GHGA-Connector to deposit files into an upload box. Upload boxes are created on request by Data Stewards and assigned to verified accounts in the GHGA Data Portal. This requires the same Crypt4GH keypair described under [Crypt4gh Keys](#crypt4gh-keys) above.

### Upload box access token
Once the access to an upload box is granted, an access token can be generated in the [User Account](https://data.ghga.de/account). An overview of upload boxes can be found in the tab "Research Data Upload". Access tokens can be generated by clicking "Create Token" and entering the public C4GH key.

There are two ways to deposit files in an upload box.

### 1. Batch-upload

Here a file 'filesToUpload.tsv' can be created, which consists of the file paths in the first column and the file aliases in the second. The file aliases should correspond to the related entries in the metadata that was provided for this submission.
...
The passphrase can be specified by the parameter ```--passphrase``` if the key is encrypted. Afterwards, the access token for the upload box has to be provided and the file submission is initialized.
...
The passphrase can be specified by the parameter ```--passphrase``` if the key is encrypted. After providing the access token, the ghga-connector connects to the upload box and provides functionality to modify it:
