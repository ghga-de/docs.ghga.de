...
To submit Research Data Files as part of a [GHGA Submission](../user_stories/submission/submitter_guide.md), the GHGA-Connector can deposit files into an upload box. Upload boxes are created on request by Data Stewards and assigned to verified accounts in the GHGA Data Portal.

### Upload box access token
Once granted access to an upload box, an access token can be generated in the [User Account](https://data.ghga.de/account). An overview of upload boxes can be found in the tab "Research Data Upload". Access tokens can be generated by clicking "Create Token" and entering the public C4GH key.
